...
\section{Results}
  Each part of the pipeline is evaluated separately. Several video sequences were recorded to evaluate segmentation and VOP features. A set of keyframe covering the whole scene from each sequence was hand-labeled to create groudtruth data. They were further projected into the 3D volume, resulting in 4176, 12346, 7583, 8916 frames for \emph{LivingRoom}, \emph{Kitchen}, \emph{Bedroom}, \emph{Desk} sequences respectively, of wich around a third was used for testing. Table \ref{fig:segm_results} summarises segmantation results. User interaction gives very accurate labeling, but it is pertinent to single object instances only. SRF prediction works quite well and is improved by the inference algorithm. Table \ref{fig:vop_results} compares VOPs against other features. The numbers reflect the percentage of correctly classified pixels with SRF using different features. VOP clearly outperforms all other features. $\Delta$ RGB mean stands for the mean difference of two randomly selected patches. 
  
  
\begin{table}[!ht]	\label{fig:segm_results}
 \center
...
\begin{table}[!ht]	\label{fig:vop_results}
 \center
...
\begin{figure}[!ht]	 \label{fig:srf_results}
 \center
...
\begin{figure}[!ht]	 \label{fig:failures}
 \center
 \includegraphics[width=0.75\textwidth]{figures/failures}
 \caption{Failure cases.}
\end{figure}

Combing colour and depth data requires careful sensor calibration. Lack thereof results in misclassification at object boundaries. Also strong changes of lighting conditions impair classification ability at certain viewpoints. What is more, no global context is used for classification, which could be improved if some prior knowledge were introduced. There are two disjoint modes: training and testing, which limits possible use cases.
...
\section{Future Work}
  ...
  - introducing prior
  - improving user interaction system (verbal priors)
  - improving scalability
  - using pure geometrical features
  - moving to outdoor settings
  - better scaling
