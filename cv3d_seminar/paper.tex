...
Unary and pairwise potentials $\phi_i(x_i)$ and $\phi_{ij}(x_i, x_j)$ encode the cost of assigning label to $v_i$ and a cost of assuming different labels by voxels in a neighbourhood, respectively. They are constatly changing due to the chainging predictions of the random forest and user interaction. 

Initially, all voxels are encouraged to take a background label by setting $\phi_i(l) = 0$ for background class and grater than zero otherwise. Later, these are replaced by the random forest predictions for each voxel and as a result of user interactions.             

When the user touches an object, a set of touched voxels $\mathcal(H)_S$ is registered. Their unary potentials are set to $\phi_i(l) = \infty$ if their labels are different then the specified one, and zero if the same, thus imposing the labeling. If user labels a region more than once, the old labeling are simply overwritten. 

To process the encircling action, the labeling is projected into the current frame. Next, a GMM is fittend with the foreground class taken as a convex hull of the encirvled region and background as the rest of the image. For every pixel in a bounding box around the user annotation the unary potential are updated as 
...
Smoothness is achieved by the standard Potts model by assigning a disconuity cost $\phi_{ij}(x_i, x_j) = \lambda_{ij}$ if voxels have different labels and zero otherwise, with $\lambda_{ij}$ a function of difference in position, intensity and normal directions.
...
\begin{thebibliography}{1}
  ...
  \bibitem{sift}
  L OWE , D. G. 1999. Object recognition from local scale-invariant features. In Proc. ICCV.
